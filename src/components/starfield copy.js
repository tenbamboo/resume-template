...
export default {
  start (canvas, height, width) {
    console.log('start', canvas, height, width)
    // Helpers
    function lineToAngle (x1, y1, length, radians) {
      var x2 = x1 + length * Math.cos(radians)
      var y2 = y1 + length * Math.sin(radians)
      return { x: x2, y: y2 }
    }
    ...
    var shootingStarEmittingInterval = 2000
    ...
    function drawShootingStar (p) {
      var x = p.x
      var y = p.y
      var currentTrailLength = (maxTrailLength * p.trailLengthDelta)
      var pos = lineToAngle(x, y, -currentTrailLength, p.getHeading())
