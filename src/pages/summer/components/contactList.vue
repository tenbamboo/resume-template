<template>
  <div class="contactListContainer cardContainer">
    <div class="cardTitle">
      联系方式
    </div>
    <div class="cardBody">
      <div class="haveData content"
        v-if="isShowContact">
        <div class="iconArea"
          @click="callPhone()">
          <img src="@/static/imgs/orginOne/icon8.png" />
          <span class="link">{{baseInfo.mobilePhone}}</span>
        </div>
        <div class="iconArea"
          @click="setClipboard(baseInfo.email)">
          <img src="@/static/imgs/orginOne/icon9.png" />
          <span class="link">{{baseInfo.email}}</span>
        </div>
        <div class="iconArea"
          @click="setClipboard(baseInfo.wechatNo)">
          <img src="@/static/imgs/orginOne/icon10.png" />
          <span class="link">{{baseInfo.wechatNo}}</span>
        </div>
        <div class="iconArea"
          @click="setClipboard(baseInfo.githubUrl)">
          <img src="@/static/imgs/orginOne/icon11.png" />
          <span class="link">{{baseInfo.githubUrl}}</span>
        </div>
      </div>
      <div class="noData content"
        v-if="!isShowContact">
        <img src="@/static/imgs/orginOne/icon12.png" />
        <p>作者暂时隐藏了联系方式，可能最近没有找工作的想法</p>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import { Vue, Prop, Component } from 'vue-property-decorator'
import Cain from '@/components/cain/'

@Component
export default class ContactList extends Vue {
  @Prop(Array) baseInfo;
  @Prop(Boolean) isShowContact;

  // 设置剪切板内容
  setClipboard (val) {
    Cain.setClipboard(val)
  }
  // 打电话
  callPhone (val) {
    Cain.callPhone(this.baseInfo.mobilePhone)
  }
}
</script>

<style scoped lang="scss">
@import "@/pages/summer/style/common.scss";
.contactListContainer {
  .noData {
    text-align: center;
    font-size: 15px;
    img {
      display: inline-block;
      width: 100px;
      height: 100px;
    }
  }
}
</style>
